...
However, we have not showed that our system satisfies Simpson's 6th requirement, that is, ``there is an intuitionistically comprehensible explanation of the meaning of the modalities relative to which [the system] is sound and complete".
%
...
%
As we have mentioned, however, different basis can be considered for intuitionistic modal logic, for example the one called \emph{constructive modal logic} $\mathsf{CK}$ and any in between.
%
These can also be studied within the birelational framework with some additional conditions, and we are convinces the fully labelled framework, once extended with these conditions, will be suitable to treat constructive fragments equally.
%
However, treating extensions of those with axioms such as we did here for Scott-Lemmon logics is known to be complex~\cite{ADS:lmcs}.
