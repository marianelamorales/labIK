...
\sonia{
The rules corresponding to reflexivity, transitivity, $F_1$ and $F_2$ are obtained through the standard axioms-to-rule procedure that is well known for geometric axioms, and applies more generally to bipolar axioms.
%
It corresponds to attributing a positive bias to both relational and pre-order atoms in the procedure established in~\cite{marin:etal:submitted}.
%
On the other hand, by polarising both relational and pre-order atoms negatively, a different shape of rules can be obtained in the style of Vigan\`o~\cite{vigano:00}. 
%
...
%
These rules are equiderivable to the one on Fig.~\ref{fig:labIKp}; note however that it is read top-down and not appropriate for proof search.	
...
Rules for $F_1$ and $F_2$ require to create new labels, and in order to be
