...
(In case some of them coincide, they are not repeated since $\B$ is a \emph{set} of atoms.)
   However, in the premise, the  expression $\lb{y'}R^{l} \lb u$ stands for  $\lb{y'}R\lb{u_1},\ldots,\lb{u_{l-1}}R \lb u$ where all $\lb{u_1},\ldots,\lb{u_{l-1}},\lb u$ are fresh, and therefore pairwise distinct, and if $l=0$ then $\lb u=\lb{y'}$; similarly for $\lb z R^{n} \lb u$, unless if $n = 0$ in which case $u=z$.
