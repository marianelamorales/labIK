...
\lutz{use the mathsf font. in the intro we also used the same for axiom and rule. So we can do the same here (just change the macro)}
...
%%
The frames that validate the axiom $\ax{4^{\DIA}} \colon \fm{\DIA\DIA A \IMP \DIA A}$ are exactly the ones satisfying the condition~$g_{2001}$: if $\accs wv$ and $\accs vu$, there exists $\lb{u'}$ s.t.~$\futs{u}{u'}$ and $\accs{w}{u'}$.
%%
The frames that validate the axiom $\ax{4^{\BOX}} \colon \fm{\BOX A \IMP \BOX \BOX A}$ are exactly the ones satisfying the condition~$g_{0120}$: if $\accs wv$ and $\accs vu$, there exists $\lb{w'}$ s.t.~$\futs{w}{w'}$ and $\accs{w'}{u}$.
...
respectively.
%
Note however that these require (relational and preorder) atoms to appear on the right-hand-side of the sequent, and the derivations are to be read top-down, and therefore are not appropriate for proof search.	
