...
%\sonia{We should not have the definitions of classical modal logic. Just the intuitionistic part. If we want to say something else about labelled sequents, we should add it before the definition of \emph{a two-sided labelled sequent}}
...
%These rule schemes can occur in different contexts and different calculi. The context that interests us is when it is applied to modal logic. The idea to extend this system to another to capture intuitionistic modal logics allows us to study the Kripke semantics for this type of logics:
...
\marianela{If it is a long paper, we can give us the space to explain the system with details. So we can add something like the next section (presenting the system by parts).}
...
\begin{itemize}
	...
	Continue with the same reasoning, $\B$ is a set of relational and pre-order atoms and $\Left$ and $\Right$ are multiset of laballed formulas. Semantically, the rule expresses, from the conclusion to the premise, that given $x, y$ and $z$ such that $x \le y$ and $yRz$ where the formula $\square A$ is true in $x$, then the formula $A$ is true in $z$.
...
\begin{center}
	$\vlinf{\sir}{$ $y$ fresh$}{\B, \Left \Rightarrow \Right, x \colon A \IMP B}{\B, \Left, x \le y, y \colon A \Rightarrow \Right, y \colon B}$
	
	
	\vspace{5mm}
	
	
	$\vliinf{\sil}{}{\B, \Left, x \le y, x \colon A \IMP B \Rightarrow \Right}{\B, \Left, x \le y, x \colon A \IMP B \Rightarrow \Right, y \colon A}{\B, \Left, x \le y, y \colon B \Rightarrow \Right}$
\end{center}
...
\begin{itemize}
	...
	Let $\B$ be a set of relational and pre-order atoms, and given $\Left$ and $\Right$ multiset of labelled formulas, this rule shows, semantically, that if the formula $A \IMP B$ is satisfied in $x$, then there exists $y$ such that $x \le y$ and the formula $A$ is true in $y$, then the formula $B$ is also true in $y$.
\end{itemize}
...
\begin{proof} [Proof for soundness]
	We need to prove that each sequent rule of our system $\labIKp$ is sound. As the proof is similar for each of the rules, we prove the soundness of some particular rules:
	
	\begin{itemize}
		\item Rule $\sbl$: 
		
		We want to prove that the rule for $\sbl$ introduced in the Figure \ref{fig:labIKp} is sound. What we want to prove is:
		
		\begin{center}
			\emph{For all model $\M$, if $\M \Vdash \B,\Left, x \le y, yRz, x \colon \square A, z \colon A \Rightarrow \Right$, then $\M \Vdash \B, \Left, x \le y, yRz, x \colon \square A \Rightarrow \Right$.}
		\end{center}
		
		By way of contradiction we want to see that if $\exists$ $\M_{0}$ such that $\M_{0} \not \Vdash$ $conclusion$,then $\M_{0} \not \Vdash premise$. More precisely for the $\sbl$ rule we want to show that if there exists a model $\M_{0}$ such that $\M_{0} \not \Vdash \B, \Left, x \le y, yRz, x \colon \square A \Rightarrow \Right$ then $\M_{0} \not \Vdash \B,\Left, x \le y, yRz, x \colon \square A, z \colon A \Rightarrow \Right$.
		
		Then assume there exists a model $\M_{0}$ such that $\M_{0} \not \Vdash \B, \Left, x \le y, yRz, x \colon \square A \Rightarrow \Right$, i.e $\M_{0} \Vdash \B$, $\M_{0} \Vdash \Left$, $\M_{0} \Vdash x \le y$, $ \M_{0} \Vdash yRz$, $\M_{0} \Vdash x\colon \square A$ and  $\M_{0} \not \Vdash \Right$. Therefore, in particular, from $\M_{0} \Vdash \square A$ we have that for all world $y, z$ such that $x \le y$ and $yRz$, then $\M_{0},z \Vdash A$. Finally, we obtain that $\M_{0} \not \Vdash \B, \Left, x \le y, yRz, x \colon \square A, z \colon A \Rightarrow \Right$.
...
\marianela{Yes! Soundness section: explain a box rule and an implication rule. Say that the scheme of the rest of the rules are similar}


\todo{How to introduce the next lemmas? Add small explanation}
...
\marianela{After Skype with Sonia: important section; add the generality of the rules for Scott-Lemmon; Simpson will be introduce in the introduction or/and in the conclusion. }

\todo{The idea of this section is to show the things that our system could do (extensions of the logic) and Simpson could not.}

\todo{Scott-Lemmon with the new rules (general rules for F2, F1, etc) for the system and the lemma that shows that each rule is admissible}
...
%
These systems enjoy cut-elimination via usual arguments. Proofs of the the axioms we mentioned are below:
...
The generality of the result adds a new rule to the system $\labIKp$ in order to capture the \emph{Scott-Lemmon axiom} or \emph{$\agklmn$ axiom}: 
\begin{center}
	$\lozenge^{k} \square^{l} A \IMP \square^{m}\lozenge^{n} A$
\end{center}
...
See this axiom in the Figure~\ref{fig:gklmn} \todo{Wrong label}
