...
\begin{proof}
	By induction on the height of $\D$, we prove that for any proof of $\lseq{\B, \futs{x}{x'}}{\Left, \labels{x}{a}}{\Right}$, there exists a proof of $\lseq{\B, \futs{x}{x'}}{\Left, \labels{x}{a}, \labels{x'}{a}}{\Right}$
	of the same (or smaller) height.
...
%		height-preserving invertible for the right premise.
	...
	\begin{proof}
		For each rule $\rn{r}$, we need to show that if there exists a proof $\DD$ of the conclusion, there exists a proof $\DD^{\rn r}_i$ of the $i$-th premisse for $i\in\{0,1,2\}$ of the same (or smaller) height.
