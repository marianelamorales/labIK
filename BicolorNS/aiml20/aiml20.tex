...
\todo{sell the systems! more emphasis in the nested sequent system?}

Structural proof theoretic accounts of intuitionistic modal logic can adopt the paradigm of \emph{labelled deduction} in the form of labelled natural deduction and labelled sequent systems~\cite{simpson1994}, or the one of \emph{unlabelled deduction} in the form of sequent~\cite{bierman2000} or nested sequent systems~\cite{strassburger2013} (for a survey see~\cite[Chap.~3]{marin2018}). In this article, we present a labelled sequent system and a nested sequent system for intuitionistic modal logics capturing the accesibility relation for modal logics and the intuitionistic relation $\le$. In the labelled calculus we made explicity both relations with $\rel$ and $\le$ into the system. In the nested sequent system, we represent this relations with one bracket in a sequent (for the accesibility relation) and a with two brackets in a sequent (preorder relation).  
...
\fm a \mid \fm{A \AND A} \mid \fm{A \OR A} \mid \fm{\TOP} \mid \fm{\BOT} \mid \fm{A \IMP A} \mid \fm{\BOX A} \mid \fm{\DIA A}
$$
%
...
\begin{definition}
	...
	\begin{enumerate}
		\item[($F_1$)] For $\lb u, \lb v, \lb{v'} \in W$, if $\accs uv$ and $\futs{v}{v'}$, there exists $\lb{u'}$ s.t.~$\futs{u}{u'}$ and $\accs{u'}{v'}$.
		
		\item[($F_2$)] For $\lb{u'}, \lb u, \lb v \in W$, if $\futs uv$, there exists $\lb{v'}$ s.t.~$\accs{u'}{v'}$ and $\futs{v}{v'}$.
	\end{enumerate}
	%	
\end{definition}
...
\begin{definition}
	A \emph{bi-relational model} $\M$ is a quadruple $\langle W, R,\le,V \rangle$ with $\langle W, R, \le \rangle$ a bi-relational frame and $V\colon W \to 2^\mathcal{A}$ a monotone valuation function, that is, a function mapping each world $\lb w$ to the subset of propositional atoms true at $\lb w$, additionally subject to:
	if $\futs{w}{w'}$ then $V(\lb w)\subseteq V(\lb{w'})$.
\end{definition}

We write $\lb w \Vdash \fm a$ if $\fm a \in V(\lb w)$, and by definition, we always have $\lb w \Vdash \fm{\TOP}$ and never that $\lb w \Vdash \fm{\BOT}$. 
%
...
$\lb w \Vdash \fm{A \AND B}$ iff $\lb w \Vdash \fm A$ and $\lb w \Vdash \fm B$

$\lb w \Vdash \fm{A \OR B}$ iff $\lb w \Vdash \fm A$ or $\lb w \Vdash \fm B$

$\lb w \Vdash \fm{A \IMP B}$ iff for all $\lb{w'}$ with $\futs{w}{w'}$, if $\lb{w'} \Vdash \fm A$ then $\lb{w'} \Vdash \fm B$

$\lb w \Vdash \fm{\BOX A}$ iff for all $\lb{w'}$ and $\lb u$ with $\futs{w}{w'}$ and $\accs{w'}{u}$, $\lb u \Vdash \fm A$ %\hfill $(\ast)$

$\lb w \Vdash \fm{\DIA A}$ iff there exists a $\lb u$ such that $\accs wu$ and $\lb u \Vdash \fm A$.

\begin{definition}
	A formula $\fm A$ is \emph{satisfied} in a model $\M = \langle W, R, \le, V \rangle$, if for all $\lb w \in W$ we have $\lb w \Vdash \fm A$.
	%
	A formula $\fm A$ is \emph{valid} in a frame $\F = \langle W, R, \le \rangle$, if for all valuations $V$, $\fm A$ is satisfied in $\langle W, R, \le, V \rangle$.
\end{definition}

Similarly to the classical case, in the case of $\IK$, the correspondence between syntax and semantics is recovered.

\begin{theorem}[Fischer-Servi~\cite{fischer1984}, Plotkin and Stirling~\cite{plotkin1986}]\label{thm:plotkin}
...
\emph{Labelled deduction} has been proposed by Gabbay~\cite{gabbay1996} in the 80s as a unifying framework throughout proof theory in order to provide proof
systems for a wide range of logics. 
%
For modal logics it can take
the form of labelled natural deduction and labelled sequent systems as used, for example, by Simpson~\cite{simpson1994}, Vigan\`o~\cite{vigano2000}, and Negri~\cite{negri2005}. 
%
...
%
The idea is to use two sorts of relational atoms, one for the modal relation $\rel$ and another one for the intuitionistic relation $\leq$ as proposed by Maffezioli, Naibo and Negri in~\cite{maffezioli2013}. 
%


\begin{definition}
	A two-sided intuitionistic \emph{labelled sequent} is of the form $\B, \Left \SEQ \Right$ where $\B$ denotes a set of relational atoms $\accs xy$ and preorder atoms $\futs xy$, and $\Left$ and $\Right$ are multi-sets of labelled formulas $\labels{x}{A}$ (for $\lb x$ and $\lb y$ taken from the set of labels and $A$ an intuitionistic modal formula).
\end{definition}
